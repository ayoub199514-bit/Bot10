...
أخرج نتيجة منظمة بالعربية على شكل JSON فقط بالحقول التالية:
...
5) أعد JSON صالحا فقط، بلا أي نص إضافي قبله أو بعده، وبلا علامات كود.
`;
...
app.post("/api/analyze", async (req, res) => {
  try {
    ...
    const match = /^data:(image\/(?:png|jpeg|jpg|webp));base64,(.+)$/.exec(imageDataUrl || "");
    if (!match) {
    ...
    if (!API_KEY) {
      return res.status(503).json({
        error: "لم يتم وضع GEMINI_API_KEY في ملف البيئة. الواجهة جاهزة، لكن التحليل الحقيقي يحتاج مفتاح API."
      });
    }
