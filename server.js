...
- multi_signals: an object with three keys "short", "medium", "long", each an object { direction: CALL/PUT/WAIT, confidence: 0-100 }, representing the likely short-term (next few candles), medium-term, and longer-term direction implied by the same chart
...
`;
    ...
    const sanitizeSignal = (s) => ({
      direction: ["CALL", "PUT", "WAIT"].includes(s?.direction) ? s.direction : "WAIT",
      confidence: Math.max(0, Math.min(100, Number(s?.confidence) || 0))
    });
    const ms = result.multi_signals || {};
    result.multi_signals = {
      short: sanitizeSignal(ms.short),
      medium: sanitizeSignal(ms.medium),
      long: sanitizeSignal(ms.long)
    };
